...
void FilterManagerImpl::onContinueReading(ActiveReadFilter* filter,
  ...
  ENVOY_LOG_MISC(info, "onContinueReading, flag:{},msg:\n{}", buffer_source.getReadBuffer().end_stream, buffer_source.getReadBuffer().buffer.toString());
  ...
  for (; entry != upstream_filters_.end(); entry++) {
    ...
    if (read_buffer.buffer.length() > 0 || read_buffer.end_stream) {
      ENVOY_LOG_MISC(info, "onContinueReading, running ........");
...
ENVOY_LOG_MISC(info, "test FilterManagerImpl::onRead");
