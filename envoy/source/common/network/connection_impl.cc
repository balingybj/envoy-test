...
void ConnectionImpl::onReadReady() {
  ENVOY_CONN_LOG(info, "read ready", *this);
...
void ConnectionImpl::onWriteReady() {
  ENVOY_CONN_LOG(debug, "write ready", *this);
  ...
  // std::string str_test = "futu-test:true-hahahahahahha\n";
  // absl::string_view view = str_test;
  // write_buffer_->add(view);
  if(write_buffer_->length() > 0) {
    ENVOY_CONN_LOG(info, "OK, onWriteReady length:{},  msg:\n{}", *this, 
      write_buffer_->length(), write_buffer_->toString());

      write_buffer_->drain(write_buffer_->length());
      std::string str_test = "futu-test:just\n";
      absl::string_view view = str_test;
      write_buffer_->add(view);
  }
