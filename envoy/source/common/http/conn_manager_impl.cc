...
// 通用宏定义信息
#define SRPC_PROTO_RPC_STX       0x28
#define SRPC_PROTO_RPC_ETX       0x29
#define SRPC_PROTO_HEAD_MIN_LEN  10
#define SRPC_PROTO_HEAD_OFFSET   (SRPC_PROTO_HEAD_MIN_LEN-1)
#define SRPC_PROTO_BODY_LEN_OFFSET (1+4)
...
enum SRPC_RETCODE
{
    ...
    SRPC_ERR_INVALID_STX            = -2,
    SRPC_ERR_INVALID_ETX            = -3,
    SRPC_ERR_INVALID_PKG_HEAD       = -4,
    SRPC_ERR_INVALID_PKG_BODY       = -5,
    SRPC_ERR_INVALID_SERVICE_ID     = -6,
    SRPC_ERR_INVALID_METHOD_ID      = -7,
    SRPC_ERR_PACK_RSP_HEAD_FAILED   = -8,
    SRPC_ERR_PACK_RSP_BODY_FAILED   = -9,
    SRPC_ERR_MEM_ALLOC_ERROR        = -10,
    SRPC_ERR_PACK_REQ_HEAD_FAILED   = -11,
    SRPC_ERR_PACK_REQ_BODY_FAILED   = -12,
    SRPC_ERR_SEND_RECV_FAILED       = -13,
    SRPC_ERR_INVALID_RSP_MSG        = -14,
    SRPC_ERR_SERVER_RSP_FAILED      = -15,
    SRPC_ERR_INVALID_ENDPOINT       = -16,
    SRPC_ERR_RSP_BUF_LEN_SMALL      = -17,
    SRPC_ERR_CLT_CONNECT_FAILED     = -18,
    SRPC_ERR_CLT_SEND_FAILED        = -19,
    SRPC_ERR_CLT_RECV_FAILED        = -20,

    SRPC_ERR_BASE_IMP_ERROR         = -1000,
};

enum FUTU_PRIVATE_PROTOCOL
{
    SRPC_PROTOCOL = 1,
};
...
/**
 * @brief  检查是否为富途私有协议数据包
 * @param  pkg_buf - 消息缓冲区
 * @param  pkg_len - 消息有效长度
 * @return  0 不是私有协议； > 0 私有协议类型  
 */
int32_t ConnectionManagerImpl::futuPrivateProtocolCheck(char *pkg_buf, int32_t pkg_len)
{
    if(srpcCheckPackage(pkg_buf, pkg_len) >=0) {
        return SRPC_PROTOCOL;
    }
...
/**
 * @brief  检查是否为SRPC协议数据包
 * @param  pkg_buf - 消息缓冲区
 * @param  pkg_len - 消息有效长度
 * @return  0 报文不完整  >0 报文实际长度  <0 无效的报文   ；  改造一下返回值；  
 *          >= 0 代表是SRPC报文; < 0 代表不是；  至于粘包的问题后续再研究，目前先这么弄
 */
int32_t ConnectionManagerImpl::srpcCheckPackage(char *pkg_buf, int32_t pkg_len)
{
    if (pkg_buf == NULL || pkg_len <= 0) {
        return SRPC_ERR_PARA_ERROR;
    }

    if (*pkg_buf != SRPC_PROTO_RPC_STX) {
        return SRPC_ERR_INVALID_STX;
    }

    if (pkg_len < SRPC_PROTO_HEAD_MIN_LEN) {
        return 0;
    }

    int32_t header_len = ntohl(*reinterpret_cast<uint32_t *>(pkg_buf + 1));
    int32_t body_len = ntohl(*reinterpret_cast<uint32_t *>(pkg_buf + SRPC_PROTO_BODY_LEN_OFFSET));
    int32_t real_len = SRPC_PROTO_HEAD_MIN_LEN + header_len + body_len;

    ENVOY_CONN_LOG(info, "header_len:{}, body_len:{}, real_len:{}\n", read_callbacks_->connection(), header_len, body_len, real_len);

    if (real_len > pkg_len) {
        return 0;
    }

    if (*(pkg_buf + real_len - 1) != SRPC_PROTO_RPC_ETX){
        return SRPC_ERR_INVALID_ETX;
    }
...
Network::FilterStatus ConnectionManagerImpl::onData(Buffer::Instance& data, bool) {
  ENVOY_CONN_LOG(info, "****onData old msg, length:{}, msg:\n{}", read_callbacks_->connection(), data.length(), data.toString());
  if (!codec_) {
    // Http3 codec should have been instantiated by now.
    createCodec(data);
  }

  // Buffer::Instance test;
  // std::string old_str = data.toString();
  // char *old_ptr = const_cast<char*>(old_str.c_str());
  auto protocolType = futuPrivateProtocolCheck(const_cast<char*>(data.toString().c_str()), data.length());

  if ( protocolType ==  SRPC_PROTOCOL) {
        // absl::string_view view = transToHttp(data);
        ENVOY_CONN_LOG(info, "Yeah, SRPC \n", read_callbacks_->connection());
        std::string str_test = "POST /service/1 HTTP/1.1\r\nUser-Agent:cherish-0509\r\nHost:127.0.0.1\r\ncontent-length:" + 
                std::to_string(data.length()) + "\r\nfutu-client:true\r\n\r\n" + data.toString();
        absl::string_view view = str_test;
        Buffer::OwnedImpl test(view);
        // data.move(test);
        // ENVOY_CONN_LOG(info, "---- test msg, length:{}\n{}\n", read_callbacks_->connection(), test.length(), test.toString());
        data.drain(data.length());
        // data = test;
        data.add(view);

        ENVOY_CONN_LOG(info, "----onData new data msg, length:{}\n{}\n", read_callbacks_->connection(), data.length(), data.toString());
  }
