...
void ConnectionImpl::flushOutput(bool end_encode) {
  ...
  ENVOY_CONN_LOG(error, "--- add test, end_encode:{} \nflshOutput:\n{}", connection_, end_encode, output_buffer_.toString());
...
int ConnectionImpl::onHeadersCompleteBase() {
  ...
  ENVOY_CONN_LOG(info, "--- ConnectionImpl::onHeadersCompleteBase", connection_);
  completeLastHeader();
...
void ServerConnectionImpl::onMessageComplete() {
  ...
  ENVOY_CONN_LOG(error, "----  http1 onMessageComplete called ", connection_);
...
void ClientConnectionImpl::onMessageComplete() {
  ENVOY_CONN_LOG(error, "---- ClientConnectionImpl message complete", connection_);
  ...
  if (pending_response_.has_value()) {
    ...
    ENVOY_CONN_LOG(error, "----  http1 response handle : ", connection_);
